...
<ol>
 ...
 <li><?php echo osc_draw_label($osC_Language->get('field_customer_first_name'), null, 'firstname', true) . osc_draw_input_field('firstname', (isset($Qentry) ? $Qentry->value('entry_firstname') : (!$osC_Customer->hasDefaultAddress() ? $osC_Customer->getFirstName() : null))); ?></li>
			...
			<?php
				...
				if ( (isset($_GET['new']) && ($_GET['new'] == 'save')) || (isset($_GET['edit']) && ($_GET['edit'] == 'save')) || (isset($_GET[$osC_Template->getModule()]) && ($_GET[$osC_Template->getModule()] == 'process')) ) {
					if ($entry_state_has_zones === true) {
						...
						echo '<span id="state">' . osc_draw_pull_down_menu('state', $zones_array) . '</span>';
					} else {
						echo '<span id="state">' . osc_draw_input_field('state') . '</span>';
					}
				} else {
					if (isset($Qentry)) {
					...
					if (isset($zones_array) && !empty($zones_array)) {
						...
						echo '<span id="state">' . osc_draw_input_field('state', (isset($Qentry) ? $zone : null)) . '</span>';
					}
				}
	 ?>
