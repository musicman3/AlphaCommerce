...
*/
  ...
  class osC_Services_debug {
    function start() {
      ...
      if (SERVICE_DEBUG_CHECK_LOCALE == '1') {
        ...
        if (($setlocale === false) || ($setlocale === null)) {
          $osC_MessageStack->add('debug', sprintf($osC_Language->get('service_debug_check_locale')) . '&nbsp;' . $osC_Language->getLocale() . ' [' . __CLASS__ . ']', 'error');
        }
      }
    ...
    function stop() {
      global $osC_MessageStack, $osC_Template, $osC_Language;
      ...
      if (!osc_empty(SERVICE_DEBUG_EXECUTION_TIME_LOG)) {
        if (!@error_log(strftime('%c') . ' - ' . $_SERVER['REQUEST_URI'] . ' (' . $parse_time . 's)' . "\n", 3, SERVICE_DEBUG_EXECUTION_TIME_LOG)) {
          if (!file_exists(SERVICE_DEBUG_EXECUTION_TIME_LOG) || !is_writable(SERVICE_DEBUG_EXECUTION_TIME_LOG)) {
            $osC_MessageStack->add('debug', sprintf($osC_Language->get('service_debug_execution_time_log')) . '&nbsp;' . SERVICE_DEBUG_EXECUTION_TIME_LOG . ' [' . __CLASS__ . ']', 'error');
          }
        }
      }

      if (SERVICE_DEBUG_EXECUTION_DISPLAY == '1') {
        $osC_MessageStack->add('debug', sprintf($osC_Language->get('service_debug_execution_display')) . '&nbsp;' . $parse_time . 's [' . __CLASS__ . ']', 'warning');
      }

      if (SERVICE_DEBUG_NUMBER_OF_QUERIES == '1') {	  
