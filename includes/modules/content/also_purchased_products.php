        ...
        while ($Qorders->next()) {
          if(($i % 3 == 0) && ($i != 0))
            ...
            if (osc_empty($Qorders->value('image')) === false) {
              $this->_content .= osc_link_object(osc_href_link(FILENAME_PRODUCTS, $Qorders->value('products_keyword')), $osC_Image->show($Qorders->value('image'), $Qorders->value('products_name')));
            }

            $this->_content .= '<span style="display:block; height: auto; text-align: center">' . osc_link_object(osc_href_link(FILENAME_PRODUCTS, $Qorders->value('products_keyword')), $Qorders->value('products_name')) .
                               '</span>';
