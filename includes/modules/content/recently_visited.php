...
/* Class constructor */
    ...
    function initialize() {
      global $osC_Services, $osC_RecentlyVisited, $osC_Language, $osC_Image, $osC_Template;
      ...
      if ($osC_Services->isStarted('recently_visited') && $osC_RecentlyVisited->hasHistory()) {
        ...
        if ($osC_RecentlyVisited->hasCategories()) {
          ...
          foreach ($osC_RecentlyVisited->getCategories() as $category) {
            $this->_content .= '<li>' . osc_image('templates/' . $osC_Template->getCode() . '/images/arrow_gray.png') . '&nbsp;' . osc_link_object(osc_href_link(FILENAME_DEFAULT, 'cPath=' . $category['path']), $category['name']);
