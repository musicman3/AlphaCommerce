...
// usps doesnt accept zero weight
    ...
    function _getQuote() {
	  ...
	  $test = $osC_Language->get('shipping_table_admin_17');
	  $production = $osC_Language->get('shipping_table_admin_18');
	  
      switch (MODULE_SHIPPING_USPS_SERVER) {
        case 12: $usps_server = 'production.shippingapis.com';
      ...
      if ($osC_ShoppingCart->getShippingAddress('country_id') == SHIPPING_ORIGIN_COUNTRY) {
        if (sizeof($response) == '1') {
          if (preg_match('/<Error>/', $response[0])) {//if (ereg('<Error>', $response[0])) {
            $number = preg_match('/<Number>(.*)<\/Number>/', $response[0], $regs);//$number = ereg('<Number>(.*)</Number>', $response[0], $regs);
            $number = $regs[1];
            $description = preg_match('/<Description>(.*)<\/Description>/', $response[0], $regs);//$description = ereg('<Description>(.*)</Description>', $response[0], $regs);
            $description = $regs[1];
        ...
        for ($i=0; $i<$n; $i++) {
          if (strpos($response[$i], '<Postage>')) {
            $service = preg_match('/<Service>(.*)<\/Service>/', $response[$i], $regs);//$service = ereg('<Service>(.*)</Service>', $response[$i], $regs);
            $service = $regs[1];
            $postage = preg_match('/<Postage>(.*)<\/Postage>/', $response[$i], $regs);//$postage = ereg('<Postage>(.*)</Postage>', $response[$i], $regs);
            $postage = $regs[1];
        ...
        if (preg_match('/<Error>/', $response[0])) {//if (ereg('<Error>', $response[0])) {
          ...
          for ($i=0, $n=$size; $i<$n; $i++) {
            if (strpos($services[$i], '<Postage>')) {
              $service = preg_match('/<SvcDescription>(.*)<\/SvcDescription>/', $services[$i], $regs);//$service = ereg('<SvcDescription>(.*)</SvcDescription>', $services[$i], $regs);
              $service = $regs[1];
              $postage = preg_match('/<Postage>(.*)<\/Postage>/', $services[$i], $regs);//$postage = ereg('<Postage>(.*)</Postage>', $services[$i], $regs);
              $postage = $regs[1];
