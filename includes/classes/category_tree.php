        ...
        foreach ( $this->_data[$parent_id] as $category_id => $category ) {
					if (SERVICES_CATEGORY_PATH_CALCULATE_PRODUCT_COUNT == '1') {
						if ($category['count'] > 0){
							...
							$result .= str_repeat($this->spacer_string, $this->spacer_multiplier * $level) . osc_image('templates/' . $osC_Template->getCode() . '/images/arrow_gray.png') . '&nbsp;' . osc_link_object(osc_href_link(FILENAME_DEFAULT, 'cPath=' . $category_link), $link_title);
