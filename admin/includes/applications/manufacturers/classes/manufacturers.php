  ...
  class osC_Manufacturers_Admin {
    ...
    public static function save($id = null, $data) {
      ...
      if ( !$osC_Database->isError() ) {
		...
		if (!file_exists(DIR_FS_CATALOG . DIR_WS_IMAGES . 'manufacturers/')) {
		mkdir(DIR_FS_CATALOG . DIR_WS_IMAGES . 'manufacturers/');
		@chmod(DIR_FS_CATALOG . DIR_WS_IMAGES . 'manufacturers/', 0777);
		}		
		...
		$width_image = $osC_Image->getWidth('thumbnails');
		$height_image = '';
		if ($width_image >0 and $height_image >0) {$width_image = 0;}
		 
		 if (isset($img)){
		$osC_Image->image_resizeicon(DIR_FS_CATALOG . DIR_WS_IMAGES . 'manufacturers/'.$img, DIR_FS_CATALOG . DIR_WS_IMAGES . 'manufacturers/'.$img, $width_image, $height_image);
}

					//	manufacturers/mini/$img	- create and resize
					if (!file_exists(DIR_FS_CATALOG . DIR_WS_IMAGES . 'manufacturers/mini/')) {
						mkdir(DIR_FS_CATALOG . DIR_WS_IMAGES . 'manufacturers/mini/');
						@chmod(DIR_FS_CATALOG . DIR_WS_IMAGES . 'manufacturers/mini/', 0777);
					}

					if (isset($img)) {
						$categories_image_mini = copy(DIR_FS_CATALOG . DIR_WS_IMAGES . 'manufacturers/' . $img, DIR_FS_CATALOG . DIR_WS_IMAGES . 'manufacturers/mini/' . $img);

						$width_image_mini = '';
						$height_image_mini = 30;
						if ($width_image_mini >0 and $height_image_mini >0) {$width_image_mini = 0;}

						$osC_Image->image_resizeicon(DIR_FS_CATALOG . DIR_WS_IMAGES . 'manufacturers/mini/'.$img, DIR_FS_CATALOG . DIR_WS_IMAGES . 'manufacturers/mini/'.$img, $width_image_mini, $height_image_mini);
					}
					// end	manufacturers/mini/$img
