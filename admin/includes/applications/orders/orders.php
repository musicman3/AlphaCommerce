      ...
      if ( !empty($_GET['action']) ) {
        switch ( $_GET['action'] ) {
          ...
          case 'delete':
					...
					case 'delete_lite':
					$this->_page_contents = 'delete_lite.php';
      ...
      if ( !$osC_Database->isError() ) {
				...
				if (EMAIL_USE_HTML == '-1') {
					if ( $data['notify_customer'] === true ) {
						...
						osc_email($Qorder->value('customers_name'), $Qorder->value('customers_email_address'), sprintf($osC_Language->get('email_subject'), $osC_Language->get('email_order_number') . ' ' . $id), $this->_encode_body($email_body), STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS);
					}
				}
